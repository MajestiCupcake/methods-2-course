...
***Answer***: take the first derivative of the function

- When having found an extremum how can you be sure it is a min or max? (think higher order derivatives.) 
***Answer***: take the second derivative at the given spot, positive = minimum and negative = maxima
...
#how to plot 3D
persp(x,y,z,phi=45,theta=-45,col="#30D5C8",shade=.00000001,ticktype="detailed")
```
...
x_plot <- seq(-10,10, by = .05)
plot(x_plot, fourmin(x_plot), type = "l")
```

  b) Find the 4 minimums using the optim() function. 
```{r}
#Optim will always find the local minimum which is the closest to the starting value with periodic function
x = -10
repeat{
  print(paste("for x = ", x," the closest minimum is at x =",optim(x, fourmin, method = "CG")$par))
  x = x +1
  if (x > 10) break("X limit reached")
}
```
...
```{r}
#zero is a mazima, using deriv() and polyroot(), else in head
...
x = -10
repeat{
  print(paste("for x = ", x," the closest maxima is at x =",optim(-x, fourmin, method = "CG")$par))
  x = x +1
  if (x > 10) break("X limit reached")
}
```
...
n <- 4
x <- rnorm(n, 20, 2) # so this is 4x1 predictor matrix 
y <- rnorm(n, mean = x *2, sd  =1 )                   # this is our outcome variable, a 4x1 vector
X_matrix <- cbind(rep(1, 100), x)      # adding a column of 1s as intercept to the design matrix X (4 x 1)
theta <- rep(0,2)               # set up the initial theta 2 x 1 vector
```
...
loss_function <-function(X, y, par){  
  n <- length(y)
  loss <- sum((X%*%par - y)^3)/(n-length(par))
return(loss) 
}
```
...
coef$par
```
We now have the two point estimates of our intercept: -9.017713e+17 and slope: -1.717772e+19.

3) It is time to start getting creative. 
  a) Create a design matrix with one predictor and simulate y from that design matrix.
```{r}
set.seed(42) #random seed to reproduce results
n <- 1
x <- rnorm(n, 20, 2) # so this is 4x1 predictor matrix 
y <- rnorm(n, mean = x *2, sd  =1 )                   # this is our outcome variable, a 4x1 vector
X_matrix <- cbind(rep(1, 100), x)      # adding a column of 1s as intercept to the design matrix X (4 x 1)
...
```{r}
cost_function1 <-function(X, y, par){  
  n <- length(y)
  cost <- sum((X%*%par - y)^20)/(n-length(par))
return(cost) 
}

cost_function2 <-function(X, y, par){  
  n <- length(y)
  cost <- sum((X%*%par - y)^3)/(n-length(par))*2
return(cost) 
}

cost_function3 <-function(X, y, par){  
  n <- length(y)
  cost <- sum((X%*%par - y)^3)*(n-length(par))
return(cost) 
}
```
...
```{r}
coef <- optim(par = theta, fn = cost_function1 , X = X_matrix, y = y, method = "BFGS")
coef$par #-2.138879e+12 -4.864293e+13

coef <- optim(par = theta, fn = cost_function2 , X = X_matrix, y = y, method = "BFGS")
coef$par #2.361311e+20 5.370213e+21

coef <- optim(par = theta, fn = cost_function3 , X = X_matrix, y = y, method = "BFGS")
coef$par #2.951694e+19 6.712780e+20
```
...
```{r}
X <-  rnorm(n, mean=y^2/2,sd=2)
par <- rnorm(n,mean=0,sd=2)

x_plot <- seq(-10,10, by = .05)
plot(x_plot, cost_function3(X=X,y=y,par=par), type = "l") #wrong
```
